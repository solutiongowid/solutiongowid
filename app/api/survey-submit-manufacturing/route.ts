...
const ZAPIER_WEBHOOK_URL = 'https://hooks.zapier.com/hooks/catch/10485854/4hjlti9/';
...
export async function POST(request: NextRequest) {
  try {
      ...
      .insert([
        {
          ...
          campaign: utm_campaign || 'manufacturing',
    ...
    if (ZAPIER_WEBHOOK_URL) {
      await fetch(ZAPIER_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          ...
          utm_campaign: utm_campaign || 'manufacturing',
          utm_content: utm_content || '',
          timestamp,
        }),
      }).catch((err) => console.error('Zapier webhook error:', err));
    } else {
      console.warn('ZAPIER_WEBHOOK_URL (manufacturing) is not set — skipping Stibee report email trigger.');
    }
    ...
    console.error('Error in survey-submit-manufacturing API:', error);
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
