...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: supabaseError } = await supabaseAdmin.from('leads').insert([
      {
        ...
        lead_source: utm_source || 'card-data-api',
        lead_source_detail: utm_medium || null,
        campaign: 'card-data-api',
        campaign_detail: utm_campaign || null,
        funnel_stage: 'new',
        lead_type: 'potential',
        notes,
        biz,
        source: trim(body.source) || 'card-data',
        name,
        company,
        phone: phone || null,
    ...
    if (supabaseError) {
      console.error('Supabase error (card-data-api-submit):', supabaseError);
      return NextResponse.json(
    ...
    console.error('Error in card-data-api-submit API:', error);
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
