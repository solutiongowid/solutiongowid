...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, company, email, phone, position, timestamp } = body;

    // 필수 필드 검증
    if (!name || !company || !email || !phone || !position) {
    ...
    if (!GOOGLE_SCRIPT_URL) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('Form Data:', { name, company, email, phone, position, timestamp });
        return NextResponse.json({ 
          success: true, 
