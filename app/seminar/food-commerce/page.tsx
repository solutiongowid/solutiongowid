...
export default function FoodCommerceWebinarPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/corporate-card-webinar-submit', {
        ...
        body: JSON.stringify({ ...formData, webinar_type: 'food-commerce', timestamp: formattedTimestamp, ...utmParams }),
      });
          ...
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                <Image
                  src="/food-commerce-webinar-thumbnail.png"
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: isWebinarEnded ? '#b0b0b0' : accentColor,
                  color: '#fff',
                  boxShadow: isWebinarEnded ? 'none' : '0 4px 20px rgba(255, 107, 53, 0.25)',
                  cursor: isWebinarEnded ? 'not-allowed' : 'pointer',
                }}
              >
                {isWebinarEnded ? '신청이 마감되었습니다' : '무료 라이브세션 신청하기'}
                ...
                {[
                  {
                    name: '고위드',
                    role: '',
                    desc: '커머스 기업 전용 법인카드·자금운영 솔루션. 푸드 48개사 재무 데이터 분석 + 푸드 브랜드 1,000곳 인터뷰.',
                    isCompany: true,
                  },
                  {
                    name: '문미성',
                    role: '고위드',
                    desc: '푸드 브랜드 1,000곳 인터뷰',
                    isCompany: false,
                  },
                  {
                    name: '김병권',
                  ...
                  <div key={i} style={{ background: '#fff', border: `1px solid ${accentBorder}`, borderRadius: '1rem', padding: '1.75rem 1.5rem' }}>
                    <div style={{ width: '3rem', height: '3rem', background: accentLight, border: `1px solid ${accentBorder}`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
                      <span style={{ fontSize: '1.25rem', fontWeight: '700', color: accentColor }}>
                        {speaker.isCompany ? 'G' : speaker.name[0]}
                      </span>
                    </div>
                    <h4 style={{ fontSize: '1.0625rem', fontWeight: '700', color: '#111', marginBottom: '0.25rem' }}>{speaker.name}</h4>
                    {speaker.role && <p style={{ fontSize: '0.875rem', color: '#888', marginBottom: '0.75rem' }}>{speaker.role}</p>}
                    <p style={{ fontSize: '0.9375rem', color: '#555', lineHeight: '1.6', margin: 0 }}>{speaker.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              ...
              <div className="cosmetic-timetable-pc" style={{ marginBottom: '3rem' }}>
                ...
                {[
                  {
                    time: '19:00 ~ 19:05',
                    title: '오프닝 — 영업흑자인데 통장은 왜 마를까',
                    speaker: '연사 전원',
                  },
                  {
                    time: '19:05 ~ 19:25',
                    session: 'Session 01',
                    title: '생존 사다리: 매출·GP·OP·현금',
                    speakerName: '문미성',
                    speakerCompany: '고위드',
                    details: [
                      '돈이 새는 네 칸의 지도 (매출→GP→OP→현금)',
                      '"진짜 진단은 숫자가 아니라 층간 갭에 있다"',
                      'GP 직접 계산법 — 내 회사 숫자로 그 자리에서',
                      '푸드 4곳 중 1곳이 팔수록 손해인 이유 (CM 마이너스 25%)',
                    ],
                  },
                  {
                    time: '19:25 ~ 19:50',
                    session: 'Session 02',
                    title: '어디서 새는가: 확장의 함정과 흑자 도산',
                    speakerName: '문미성 · 김병권',
                    speakerCompany: '고위드',
                    details: [
                      '매출 100~200억, 가장 많이 무너지는 구간 (Decline 66.7%)',
                      '같은 매출, 구조만 다른 두 회사 — A사 vs B사 시뮬레이션',
                      '매출 1원이 거치는 세 지갑 (배달앱 7일·새벽배송 60일·대형유통 90일)',
                      '런웨이 직접 계산 — 내 회사는 몇 개월 버티나',
                    ],
                  },
                  {
                    time: '19:50 ~ 20:05',
                    ...
                    speakerCompany: '고위드',
                    details: [
                      '네 칸을 다 막은 공식: 공헌이익 31.7% · 런웨이 18.6개월',
                      '3대 변곡점: 데뷔 구간 / 확장의 함정 / B2B 안정화',
                      '현장 자가진단 — 내 회사는 어느 칸에서 새나',
                    ],
                  },
                  {
                    time: '20:05 ~ 20:15',
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '1rem', padding: '1.25rem 0', borderBottom: '1px solid #e0e0e0', alignItems: item.details ? 'start' : 'center' }}>
                    ...
                    <div>
                      ...
                      {item.speaker && !item.speakerName && (
                        <div style={{ fontSize: '0.875rem', color: '#888' }}>{item.speaker}</div>
                      )}
                      {item.details && (
              ...
              <div className="cosmetic-timetable-mobile" style={{ marginBottom: '3rem' }}>
                {[
                  ...
                  <div key={i} style={{ padding: '1.25rem 0', borderBottom: '1px solid #e0e0e0' }}>
                    ...
                    {item.speaker && !item.speakerName && (
                      <div style={{ fontSize: '0.8125rem', color: '#888' }}>{item.speaker}</div>
                    )}
                    {item.details && (
                      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        {item.details.map((d, j) => (
