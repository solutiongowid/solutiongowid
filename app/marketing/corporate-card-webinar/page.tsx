...
export default function CorporateCardWebinarMarketingPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/corporate-card-webinar-submit', {
        ...
        body: JSON.stringify({ ...formData, webinar_type: 'marketing', timestamp: formattedTimestamp, ...utmParams }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || '제출에 실패했습니다.');

      setIsSuccess(true);
      setFormData({ name: '', companyName: '', department: '', position: '', annualRevenue: '', email: '', phone: '', question: '', serviceInterest: [] as string[], agreePrivacy: false });
    } catch (error) {
  ...
  const accentColor = '#7B5BB5';
  const accentLight = 'rgba(123, 91, 181, 0.08)';
  const accentBorder = 'rgba(123, 91, 181, 0.18)';
  ...
  return (
    <>
      <div className="report-page" style={{ background: '#ffffff', color: '#222' }}>
        ...
        {/* 히어로 섹션 */}
        <header className="report-header" style={{ background: 'linear-gradient(180deg, #f3eff8 0%, #ffffff 100%)', padding: '8rem 0 5rem' }}>
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              <div style={{ marginBottom: '1.5rem' }}>
                <span className="badge" style={{ fontSize: '0.875rem', padding: '0.5rem 1.25rem', background: accentLight, color: accentColor, border: `1px solid ${accentBorder}` }}>5월 21일 (목) 오후 5시 | 온라인 웨비나</span>
              </div>
              <h1 className="report-h1" style={{ textAlign: 'center', marginBottom: '1.5rem', color: '#111' }}>
                <strong>
                  2026 법인카드 —<br />여기까지 왔습니다<br />: 광고·마케팅편
                </strong>
              </h1>
              ...
              <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                <Image
                  src="/corporate-card-webinar-marketing-thumbnail.png"
                  alt="2026 법인카드 — 여기까지 왔습니다 : 광고·마케팅편 - GoWid 웨비나"
                  width={1200}
                  height={630}
                  priority
        ...
        {/* 행사 소개 */}
        <section className="report-section" style={{ background: '#f5f1fa', padding: '5rem 0' }}>
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                법인카드를 바꾸기로 결정한 광고·마케팅 대표들한테<br className="mobile-only-br" /> 나중에 물어보면,<br />
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                광고주 결제 60~90일 vs 미디어비 즉시 결제,<br />Meta·Google 해외 결제 수수료,<br />Q4 캠페인 피크 자금 압박 —<br />
                어떤 조건에서 바꾸는 게 맞고,<br />
                어떤 조건에서는 지금 그대로여도 되는지.
              </p>
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9' }}>
                <strong>광고·마케팅 활성 법인 데이터 기반으로 그 기준을 40분 안에 다룹니다.</strong>
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '640px', margin: '0 auto', textAlign: 'left' }}>
                {[
                  '같은 매출 50억 광고대행인데 법인카드 한도가 3배 차이 나는 이유',
                  '광고주 결제 60일·미디어비 즉시 결제 사이 매출채권에 묶이는 자금을 받쳐주는 카드 조합',
                  'Meta·Google 해외 결제 수수료를 매월 마진으로 회수하는 결제 방식',
                  '비슷한 규모의 광고·마케팅 에이전시들이 돈을 실제로 어디에 쓰고 있는지',
                ].map((text, i) => (
        ...
        {/* 이런 분들께 추천합니다 */}
        <section className="report-section" style={{ background: '#f5f1fa' }}>
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxWidth: '600px', margin: '0 auto' }}>
                {[
                  <><strong>취급고는 빠르게 커지는데 카드 한도가 따라가지 못해 미디어비 결제가 막히는</strong> 광고대행 대표</>,
                  <>한도 견적 받아봤지만 <strong>캠페인 한 건도 못 받쳐서</strong> 그대로 둔 분</>,
                  <><strong>광고주 결제 60일·미디어비 즉시 결제로 매월 매출채권에 자금이 묶이는</strong> 재무팀</>,
                  <>카드를 바꿔야 한다는 건 알지만 <strong>&ldquo;지금 굳이?&rdquo;로 미뤄온</strong> 에이전시 대표</>,
                  <>바꿀 마음은 있지만 <strong>이전 자체가 부담</strong>이라 미뤄온 대표</>,
                ].map((text, i) => (
              ...
              <button onClick={openModal} className="report-button-primary" style={{ marginTop: '2.5rem', background: accentColor, color: '#fff', boxShadow: '0 4px 20px rgba(123, 91, 181, 0.25)' }}>
                무료 웨비나 신청하기
              </button>
            </div>
          </div>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              ...
              <div className="cosmetic-timetable-pc" style={{ marginBottom: '3rem' }}>
                ...
                {[
                  ...
                  {
                    time: '00:03 ~ 00:13', session: 'Session 01', title: '같은 매출, 다른 한도 — 그 격차의 정체', speakerName: '문미성 리드', speakerCompany: '고위드',
                    details: ['같은 매출 50억 광고대행인데 한도가 3배 차이 나는 두 회사 — 뭐가 다른가', '한도 산정·매출채권·해외 결제 — 3년 사이 시장이 벌려놓은 격차', '광고·마케팅 N개사 데이터로 본 법인카드 이용 트렌드'],
                    highlight: '대부분은 "몰랐다"가 아니라 "확인을 안 했을 뿐"',
                  },
                  {
                    time: '00:13 ~ 00:25', session: 'Session 02', title: '안 바꾸는 동안 통장에서 빠지고 있는 돈', speakerName: '문미성 리드', speakerCompany: '고위드',
                    details: ['광고주 결제 60~90일 vs 미디어비 즉시 결제 — CCC 음수 -45~-60일이 기본값', '인당매출 1.5~2억 임계 — 인건비가 매출의 60~80%, 매출 늘어도 마진은 그대로', 'Q4 캠페인 피크 시점 자금 압박 — 한 분기에 몰리는 미디어비 결제'],
                    highlight: 'Meta·Google·TikTok 해외 결제 — 매월 빠져나가는 환차손과 해외 수수료',
                  },
                  {
                    time: '00:25 ~ 00:32', session: 'Session 03', title: '세 가지 사례로 보는 — 같은 매출, 다른 현금흐름', speakerName: '김병권 BD', speakerCompany: '고위드',
                    details: ['광고주 결제와 미디어비 즉시 결제 시차에 자금이 묶이던 브랜드 — 결제 사이클 53일·87일로 광고주 회수 시점과 정렬, 매입액 3배 이상', '매체 직거래·외주 인건비를 카드로 전환한 브랜드 — 매입대금 후불화로 외주 인건비까지 카드 결제 효과로 유예', '해외 매체 결제 수수료가 마진을 갉아먹던 브랜드 — 해외 결제 전용 카드 전환으로 매월 빠지던 수수료 회수, 캠페인 마진 회복'],
                    highlight: '세 케이스의 공통점 — 한번 봐야겠다 싶었을 때, 그냥 확인했을 뿐',
                  },
                  {
                    time: '00:32 ~ 00:40', session: 'Session 04', title: '지금 바꿔야 하나, 아직 그대로여도 되나', speakerName: '문미성 리드', speakerCompany: '고위드',
                    details: ['바꿔야 할 회사의 시그널 5가지', '아직 안 바꿔도 되는 회사의 조건 — 양면 제시', '자가 점검 체크리스트 · 전환 3스텝'],
                    highlight: '오늘의 한 가지 — 한도 조회 10분',
                  },
                ].map((item, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '1rem', padding: '1.25rem 0', borderBottom: '1px solid #e0e0e0', alignItems: item.details ? 'start' : 'center' }}>
                    ...
                    <div>
                      ...
                      {item.speakerName && (
                        <div style={{ fontSize: '0.875rem', color: '#888', marginBottom: item.details ? '0.75rem' : '0' }}>{item.speakerName} · {item.speakerCompany}</div>
                      )}
                      {item.details && (
              ...
              <div className="cosmetic-timetable-mobile" style={{ marginBottom: '3rem' }}>
                {[
                  ...
                  <div key={i} style={{ padding: '1.25rem 0', borderBottom: '1px solid #e0e0e0' }}>
                    ...
                    <div style={{ fontSize: '0.9375rem', fontWeight: '600', color: '#222', marginBottom: item.details ? '0.5rem' : '0' }}>{item.title}</div>
                    {item.speakerName && (
                      <div style={{ fontSize: '0.8125rem', color: '#888', marginBottom: item.details ? '0.75rem' : '0' }}>{item.speakerName} · {item.speakerCompany}</div>
                    )}
                    {item.details && (
                      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        {item.details.map((d, j) => (
          ...
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                {[
                  { icon: '\u{1F4C4}', title: '광고·마케팅 카드 사용\n벤치마크 리포트 2026', desc: 'PDF 무료 제공' },
          ...
          <div className="report-container">
            <div style={{ maxWidth: '640px', margin: '0 auto' }}>
              ...
              {[
                ...
                { q: '어떤 업종에 적합한 웨비나인가요?', a: '광고·마케팅 에이전시를 중심으로 구성했지만, 광고주 결제와 미디어비 즉시 결제·해외 매체 수수료·캠페인 피크 자금 등 광고/마케팅 업의 현금 흐름 이슈가 있다면 카테고리 불문하고 도움이 됩니다.' },
        ...
        {/* 최종 CTA */}
        <section className="report-section report-section-cta" style={{ background: '#f3eff8', textAlign: 'center' }}>
          <div className="report-container">
            <div className="report-cta-content">
              <h3 className="report-h3" style={{ color: '#111' }}>
                <strong>바꿀 타이밍인지,<br />40분이면 답이 나옵니다.</strong>
              </h3>
              <button onClick={openModal} className="report-button-cta" style={{ background: accentColor, color: '#fff', boxShadow: '0 4px 20px rgba(123, 91, 181, 0.25)' }}>
                무료 웨비나 신청하기
              </button>
            </div>
          </div>
        ...
        {/* 푸터 */}
        <footer className="report-footer" style={{ background: '#f5f1fa', borderTop: '1px solid #e0e0e0' }}>
          <div className="report-container">
            ...
            {isSuccess ? (
              ...
              <>
                <div className="modal-header">
                  ...
                  <p className="modal-description">5월 21일(목) 오후 5시 | 2026 법인카드 — 여기까지 왔습니다 : 광고·마케팅편</p>
                </div>
