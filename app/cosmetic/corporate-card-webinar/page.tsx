...
export default function CorporateCardWebinarCosmeticPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/corporate-card-webinar-submit', {
        ...
        body: JSON.stringify({ ...formData, webinar_type: 'cosmetic', timestamp: formattedTimestamp, ...utmParams }),
      });
  ...
  return (
    <>
      <div className="report-page" style={{ background: '#ffffff', color: '#222' }}>
        ...
        {/* 히어로 섹션 */}
        <header className="report-header" style={{ background: 'linear-gradient(180deg, #fdf2f5 0%, #ffffff 100%)', padding: '8rem 0 5rem' }}>
          <div className="report-container">
        ...
        {/* 행사 소개 */}
        <section className="report-section" style={{ background: '#fef5f8', padding: '5rem 0' }}>
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                법인카드를 바꾸기로 결정한 D2C 뷰티 대표들한테<br className="mobile-only-br" /> 나중에 물어보면,<br />
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                광고비 ROAS 변동, OEM·부자재 매입,<br />올리브영·쿠팡 정산 시차 —<br />
                어떤 조건에서 바꾸는 게 맞고,<br />
                어떤 조건에서는 지금 그대로여도 되는지.
              </p>
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9' }}>
                <strong>178개 뷰티 브랜드 협업 데이터 기반으로 그 기준을 40분 안에 다룹니다.</strong>
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '640px', margin: '0 auto', textAlign: 'left' }}>
                {[
                  '같은 매출 50억 D2C 뷰티 브랜드인데 법인카드 한도가 3배 차이 나는 이유',
                  'ROAS 한 분기 어긋났을 때 현금흐름이 무너지는 구조와 그걸 받쳐주는 카드 조합',
                  '공병 6개월치를 한 번에 매입할 때 통장이 마이너스로 떨어지지 않게 만드는 결제 방식',
                  '비슷한 규모의 D2C 뷰티 브랜드들이 돈을 실제로 어디에 쓰고 있는지',
                ].map((text, i) => (
        ...
        {/* 이런 분들께 추천합니다 */}
        <section className="report-section" style={{ background: '#fef5f8' }}>
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxWidth: '600px', margin: '0 auto' }}>
                {[
                  <><strong>매출은 두 배로 늘었는데 카드 한도는 작년과 똑같은</strong> D2C 뷰티 대표</>,
                  <>한도 견적 받아봤지만 <strong>광고 한 분기도 못 받쳐서</strong> 그대로 둔 분</>,
                  <>올리브영·쿠팡·자사몰 <strong>정산 사이클 차이</strong>로 광고 집행과 회수가 어긋나는 재무팀</>,
                  <>카드를 바꿔야 한다는 건 알지만 <strong>&ldquo;지금 굳이?&rdquo;로 미뤄온</strong> 코스메틱 대표</>,
              ...
              <div className="cosmetic-timetable-pc" style={{ marginBottom: '3rem' }}>
                ...
                {[
                  { time: '00:00 ~ 00:03', title: '오프닝', speaker: '문미성 리드 · 고위드' },
                  {
                    ...
                    details: ['같은 매출 50억 D2C 뷰티인데 한도가 3배 차이 나는 두 회사 — 뭐가 다른가', '한도 산정·정산 사이클·해외 결제 — 3년 사이 시장이 벌려놓은 격차', 'D2C 뷰티 N개사 데이터로 본 법인카드 이용 트렌드'],
                  ...
                  },
                  {
                    ...
                    details: ['광고비 ROAS 변동성이 만드는 운전자본 묶임', '올리브영·쿠팡 정산 시차(45~60일)와 광고비 즉시 결제가 어긋날 때 생기는 현금 공백', '공병·부자재 6개월치를 한 번에 매입할 때 무너지는 통장'],
                    highlight: '채널별 공헌이익률 — 자사몰 39%·스마트스토어 33%·쿠팡 29%·올리브영 13%, 채널 mix가 결정하는 운전자본',
                  },
                  {
                    time: '00:25 ~ 00:32', session: 'Session 03', title: '세 가지 사례로 보는 — 같은 매출, 다른 현금흐름', speakerName: '김병권 BD', speakerCompany: '고위드',
                    details: ['광고비 ROAS 변동에 한도가 따라가지 못했던 브랜드 — 변동비 결제 유예로 광고 끊김 0건, 카드 매입액 10배 이상', 'OEM·부자재 매입에 자금이 한꺼번에 빠지던 브랜드 — 후불화 + 공병 12개월 할부로 시즌 자금 압박 해소', '채널 정산 시차에 자금이 묶이던 브랜드 — 신용공여일 53일·87일로 채널별 정산 사이클 흡수'],
        ...
        {/* 최종 CTA */}
        <section className="report-section report-section-cta" style={{ background: '#fdf2f5', textAlign: 'center' }}>
          <div className="report-container">
        ...
        {/* 푸터 */}
        <footer className="report-footer" style={{ background: '#fef5f8', borderTop: '1px solid #e0e0e0' }}>
          <div className="report-container">
