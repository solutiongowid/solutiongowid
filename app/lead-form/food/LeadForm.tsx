...
interface FoodLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function FoodLeadForm({ utmParams }: FoodLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-food-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/food/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
