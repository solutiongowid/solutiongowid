...
interface FashionLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function FashionLeadForm({ utmParams }: FashionLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-fashion-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/fashion/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="lead-form-card-form">
        ...
        <div style={{ position: 'relative', zIndex: 10, pointerEvents: isSubmitting ? 'none' : 'auto' }}>
          <button
            ...
            style={{ background: isSubmitting ? '#ccc' : 'linear-gradient(135deg, #329451 0%, #1F6B3D 100%)', boxShadow: isSubmitting ? 'none' : '0 4px 16px rgba(50, 148, 81, 0.3)' }}
          >
