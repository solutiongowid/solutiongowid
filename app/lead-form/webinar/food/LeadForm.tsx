...
interface WebinarFoodLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function WebinarFoodLeadForm({ utmParams }: WebinarFoodLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-webinar-food-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/webinar/food/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="lead-form-card-form">
        ...
        <div className="form-group">
          ...
          >
            ...
            <option value="100~200억">100~200억</option>
        ...
        <div style={{ position: 'relative', zIndex: 10, pointerEvents: isSubmitting ? 'none' : 'auto' }}>
          <button
            ...
            style={{ background: isSubmitting ? '#ccc' : 'linear-gradient(135deg, #FF6B35 0%, #e55a25 100%)', boxShadow: isSubmitting ? 'none' : '0 4px 16px rgba(255, 107, 53, 0.3)' }}
          >
            {isSubmitting ? '제출 중...' : '녹화본 받기'}
          </button>
        </div>
      </form>
    </div>
