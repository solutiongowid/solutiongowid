...
interface WebinarCosmeticLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function WebinarCosmeticLeadForm({ utmParams }: WebinarCosmeticLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-webinar-cosmetic-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/webinar/cosmetic/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
  ...
  return (
    <div className="lead-form-card">
      <div className="lead-form-card-header">
        <h2 className="lead-form-card-title">라운드테이블 다시보기 신청</h2>
        <p className="lead-form-card-description">
          아래 정보를 입력하시면 라운드테이블 녹화본을 바로 확인하실 수 있습니다.
        </p>
      </div>
