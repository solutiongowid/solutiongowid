...
interface AiSaasLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function AiSaasLeadForm({ utmParams }: AiSaasLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-ai-saas-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/ai-saas/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="lead-form-card-form">
        ...
        <div style={{ position: 'relative', zIndex: 10, pointerEvents: isSubmitting ? 'none' : 'auto' }}>
          <button
            ...
            style={{ background: isSubmitting ? '#ccc' : 'linear-gradient(135deg, #3B82F6 0%, #1D4ED8 100%)', boxShadow: isSubmitting ? 'none' : '0 4px 16px rgba(59, 130, 246, 0.3)' }}
          >
