...
interface B2bSaasLeadFormProps {
  utmParams: { utm_source: string; utm_medium: string; utm_campaign: string; utm_content: string };
}

export default function B2bSaasLeadForm({ utmParams }: B2bSaasLeadFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/lead-form-b2b-saas-submit', {
        method: 'POST',
      ...
      router.push(`/lead-form/b2b-saas/thank-you${qs ? `?${qs}` : ''}`);
    } catch (error) {
