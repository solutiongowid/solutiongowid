...
export default function CorporateCardWebinarFnbPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/corporate-card-webinar-submit', {
        ...
        body: JSON.stringify({ ...formData, webinar_type: 'fnb', timestamp: formattedTimestamp, ...utmParams }),
      });
  ...
  const accentColor = '#C2553A';
  const accentLight = 'rgba(194, 85, 58, 0.08)';
  const accentBorder = 'rgba(194, 85, 58, 0.18)';
  ...
  return (
    <>
      <div className="report-page" style={{ background: '#ffffff', color: '#222' }}>
        ...
        {/* 히어로 섹션 */}
        <header className="report-header" style={{ background: 'linear-gradient(180deg, #fdf1ee 0%, #ffffff 100%)', padding: '8rem 0 5rem' }}>
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              <div style={{ marginBottom: '1.5rem' }}>
                <span className="badge" style={{ fontSize: '0.875rem', padding: '0.5rem 1.25rem', background: accentLight, color: accentColor, border: `1px solid ${accentBorder}` }}>5월 20일 (수) 오후 5시 | 온라인 웨비나</span>
              </div>
              <h1 className="report-h1" style={{ textAlign: 'center', marginBottom: '1.5rem', color: '#111' }}>
                <strong>
                  2026 법인카드 —<br />여기까지 왔습니다<br />: F&amp;B편
                </strong>
              </h1>
              ...
              <div style={{ maxWidth: '600px', margin: '0 auto' }}>
                <Image
                  src="/corporate-card-webinar-fnb-thumbnail.png"
                  alt="2026 법인카드 — 여기까지 왔습니다 : F&B편 - GoWid 웨비나"
                  width={1200}
                  height={630}
                  priority
        ...
        {/* 행사 소개 */}
        <section className="report-section" style={{ background: '#fdf3f0', padding: '5rem 0' }}>
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                법인카드를 바꾸기로 결정한 외식·식음 대표들한테<br className="mobile-only-br" /> 나중에 물어보면,<br />
              ...
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9', marginBottom: '1.5rem' }}>
                식자재·부자재 단가 변동, 카드 매출 D+30 vs<br />임대료·인건비 즉시 결제, 매장 확장 시점 자금 압박 —<br />
                어떤 조건에서 바꾸는 게 맞고,<br />
                어떤 조건에서는 지금 그대로여도 되는지.
              </p>
              <p style={{ fontSize: '1.0625rem', color: '#333', lineHeight: '1.9' }}>
                <strong>F&amp;B 활성 법인 데이터 기반으로 그 기준을 40분 안에 다룹니다.</strong>
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="report-container">
            <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '640px', margin: '0 auto', textAlign: 'left' }}>
                {[
                  '같은 매출 50억 식음 브랜드인데 법인카드 한도가 3배 차이 나는 이유',
                  '원두·부자재 단가 폭증 시점에 통장이 마이너스로 떨어지지 않게 만드는 결제 방식',
                  '3호점·5호점 진입 직전 자금 압박을 받쳐주는 카드 조합',
                  '비슷한 규모의 외식·식음 브랜드들이 돈을 실제로 어디에 쓰고 있는지',
                ].map((text, i) => (
        ...
        {/* 이런 분들께 추천합니다 */}
        <section className="report-section" style={{ background: '#fdf3f0' }}>
          <div className="report-container">
            <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxWidth: '600px', margin: '0 auto' }}>
                {[
                  <><strong>매장이 늘어날수록 식자재·임대료 결제일과 매출 정산 시점이 어긋나는</strong> F&B 대표</>,
                  <>한도 견적 받아봤지만 <strong>식자재 한 달치도 못 받쳐서</strong> 그대로 둔 분</>,
                  <>카드 매출 D+30·임대료·인건비 즉시 결제로 <strong>매월 통장이 마이너스</strong>로 떨어지는 재무팀</>,
                  <>카드를 바꿔야 한다는 건 알지만 <strong>&ldquo;지금 굳이?&rdquo;로 미뤄온</strong> 외식·식음 대표</>,
              ...
              <button onClick={openModal} className="report-button-primary" style={{ marginTop: '2.5rem', background: accentColor, color: '#fff', boxShadow: '0 4px 20px rgba(194, 85, 58, 0.25)' }}>
                무료 웨비나 신청하기
              </button>
            </div>
          </div>
              ...
              <div className="cosmetic-timetable-pc" style={{ marginBottom: '3rem' }}>
                ...
                {[
                  { time: '00:00 ~ 00:03', title: '오프닝', speaker: '문미성 리드 · 고위드' },
                  {
                    ...
                    details: ['같은 매출 50억 식음 브랜드인데 한도가 3배 차이 나는 두 회사 — 뭐가 다른가', '한도 산정·정산 사이클·매장 단위경제 — 3년 사이 시장이 벌려놓은 격차', 'F&B N개사 데이터로 본 법인카드 이용 트렌드'],
                  ...
                  },
                  {
                    ...
                    details: ['식자재·부자재 단가 변동성이 만드는 운전자본 묶임 (원두·일회용 컵·포장용기 폭증)', '카드 매출 D+30 vs 임대료·인건비·식자재 즉시 결제 — 매월 30~50일 음수 운전자본', '매장 확장 시점(3호점·5호점·10호점) 일시적 자금 폭증'],
                    highlight: '채널별 정산 시차 — 매장 즉시·배달앱 30일·B2B 납품 60~90일',
                  },
                  {
                    time: '00:25 ~ 00:32', session: 'Session 03', title: '세 가지 사례로 보는 — 같은 매출, 다른 현금흐름', speakerName: '김병권 BD', speakerCompany: '고위드',
                    details: ['식자재 단가 폭증에 한도가 따라가지 못했던 브랜드 — 매입대금 후불화로 단가 피크 분기에도 매장 운영 유지', '매장 확장 시점에 자금이 한꺼번에 빠지던 브랜드 — 시즌 한시 한도 증액으로 출점 시점 자금 압박 해소', '카드 매출과 임대료·인건비 결제 시차에 자금이 묶이던 브랜드 — 고정 결제 카드 전환으로 운전자본 양수 전환'],
          ...
          <div className="report-container">
            <div style={{ maxWidth: '640px', margin: '0 auto' }}>
              ...
              {[
                ...
                { q: '어떤 업종에 적합한 웨비나인가요?', a: '외식·식음 브랜드를 중심으로 구성했지만, 식자재 단가 변동·매장 확장 자금·매출과 결제 시차 등 F&B 업의 현금 흐름 이슈가 있다면 카테고리 불문하고 도움이 됩니다.' },
        ...
        {/* 최종 CTA */}
        <section className="report-section report-section-cta" style={{ background: '#fdf1ee', textAlign: 'center' }}>
          <div className="report-container">
            <div className="report-cta-content">
              <h3 className="report-h3" style={{ color: '#111' }}>
                <strong>바꿀 타이밍인지,<br />40분이면 답이 나옵니다.</strong>
              </h3>
              <button onClick={openModal} className="report-button-cta" style={{ background: accentColor, color: '#fff', boxShadow: '0 4px 20px rgba(194, 85, 58, 0.25)' }}>
                무료 웨비나 신청하기
              </button>
            </div>
          </div>
        ...
        {/* 푸터 */}
        <footer className="report-footer" style={{ background: '#fdf3f0', borderTop: '1px solid #e0e0e0' }}>
          <div className="report-container">
            ...
            {isSuccess ? (
              ...
              <>
                <div className="modal-header">
                  ...
                  <p className="modal-description">5월 20일(수) 오후 5시 | 2026 법인카드 — 여기까지 왔습니다 : F&B편</p>
                </div>
